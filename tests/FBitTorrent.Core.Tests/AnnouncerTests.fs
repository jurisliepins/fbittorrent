...
type AnnouncerTests() = 
    ...
    let announceCommand =
        Announcer.Announce (Constants.Announce, [||], [||], 0, 0L, 0L, 0L, (Some Tracker.Event.Started), None)
    
    let scheduleAnnounceCommand =
        Announcer.ScheduleAnnounce (Constants.Announce, [||], [||], 0, 0L, 0L, 0L, (Some Tracker.Event.Started), None, 0)
